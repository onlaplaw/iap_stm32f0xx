...
/**
  * @brief  This is a function of erasing one page
  * @param  StartSector: start of user flash area
  * @retval 0: user flash area successfully erased
  *         1: error occurred
  */
void Erase_One_FlashPage(uint32_t ADDR)
{
    uint8_t i;
    volatile FLASH_Status MY_FLASHStatus = FLASH_COMPLETE;
    FLASH_Unlock();               //解锁Flash
    FLASH_ClearFlag(FLASH_FLAG_EOP|FLASH_FLAG_WRPERR | FLASH_FLAG_PGERR | FLASH_FLAG_BSY);     //	清零所有的挂起标志
    for(i = 0; (i < 1) && (MY_FLASHStatus == FLASH_COMPLETE); i++)      //擦除操作
    {	MY_FLASHStatus = FLASH_ErasePage(ADDR);}                        //擦除Address 所在的 page 页
    FLASH_Lock();                 //Flash 锁定
}
/**
  * @brief  This function is to write data to the specified address
  * @param  StartSector: start of user flash area
  * @retval 0: user flash area successfully erased
  *         1: error occurred
  */

void Write_In_To_Flash(uint32_t ADDR,uint32_t dat)
{
    FLASH_Unlock();  //解锁Flash
    FLASH_ClearFlag(FLASH_FLAG_PGERR | FLASH_FLAG_EOP);    //	清零所有的挂起标志
    /*********************************************************************************
         //               FLASH_FLAG_BSY            FLASH_BSY
         //               FLASH_FLAG_EOP            FLASH_End of Operation flag
         //               FLASH_FLAG_PGERR          FLASH_Program error flag
         //               FLASH_FLAG_WRPRTERR       FLASH_Write protected error flag        
    **********************************************************************************/
    FLASH_ProgramWord(ADDR,dat); //ProgramHalfWord 
    FLASH_ClearFlag(FLASH_FLAG_EOP|FLASH_FLAG_WRPERR | FLASH_FLAG_PGERR | FLASH_FLAG_BSY); //	清零所有的挂起标志
...
/**
  ...
  *         1: error occurred
  */
uint32_t Read_D(uint32_t ADDR,uint8_t* Read_Buf,uint32_t Read_Len){
    int Num=0;
    while(Num<Read_Len){
        *(Read_Buf +Num) = *(__IO uint8_t*)ADDR++;
        Num++;
    }
    return Num;
}
uint32_t Read_Flash_Data(uint32_t ADDR){
    uint8_t temp[4];
    uint32_t dat;
    Read_D(ADDR,temp,4);
    dat = ((uint32_t)(temp[3])<<24)|((uint32_t)(temp[2])<<16)|((uint32_t)(temp[1])<<8)|((uint32_t)(temp[0]));
    return dat;
}
/**
